...
bool string_obj::operator==(const string_obj &rhs) const {
...
string_obj string_obj::operator-() const {
...
//a loop that goes until it is less than or equal to the rhs int. Takes dereferenced this pointer and assigns it to 
//temp object adding it to the temp object. 
 string_obj string_obj::operator*(int rhs) {
    string_obj temp;
    for(int i = 1; i<=rhs; i++) {
        temp = temp + *this; 
    }
 ...
 //to make entire string object uppercase
  string_obj &string_obj::operator++(const string_obj &obj) {
...
//used. 
string_obj string_obj::operator++(int){
    string_obj temp {*this}; 
    operator++(); 
    return temp; 
}
